...
from opengauss_mcp.sql.safe_sql import SafeSqlDriver
from opengauss_mcp.sql.sql_driver import DbConnPool
...
async def test_get_sql_driver_returns_correct_driver(access_mode, expected_driver_type, mock_db_connection):
    ...
    with (
        patch("opengauss_mcp.server.current_access_mode", access_mode),
        patch("opengauss_mcp.server.db_connection", mock_db_connection),
    ):
...
async def test_get_sql_driver_sets_timeout_in_restricted_mode(mock_db_connection):
    ...
    with (
        patch("opengauss_mcp.server.current_access_mode", AccessMode.RESTRICTED),
        patch("opengauss_mcp.server.db_connection", mock_db_connection),
    ):
        driver = await get_sql_driver()
        assert isinstance(driver, SafeSqlDriver)
...
async def test_get_sql_driver_in_unrestricted_mode_no_timeout(mock_db_connection):
    ...
    with (
        patch("opengauss_mcp.server.current_access_mode", AccessMode.UNRESTRICTED),
        patch("opengauss_mcp.server.db_connection", mock_db_connection),
    ):
        driver = await get_sql_driver()
...
async def test_command_line_parsing():
    ...
    try:
        ...
        with (
            patch("opengauss_mcp.server.current_access_mode", AccessMode.UNRESTRICTED),
            patch("opengauss_mcp.server.db_connection.pool_connect", AsyncMock()),
            patch("opengauss_mcp.server.mcp.run_stdio_async", AsyncMock()),
            patch("opengauss_mcp.server.shutdown", AsyncMock()),
        ):
            # Reset the current_access_mode to UNRESTRICTED
            import opengauss_mcp.server

            opengauss_mcp.server.current_access_mode = AccessMode.UNRESTRICTED
            ...
            assert opengauss_mcp.server.current_access_mode == AccessMode.RESTRICTED
