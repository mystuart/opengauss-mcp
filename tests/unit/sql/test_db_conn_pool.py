...
async def test_pool_connect_success(mock_pool):
    ...
    with patch("opengauss_mcp.sql.sql_driver.AsyncConnectionPool", return_value=mock_pool):
...
async def test_pool_connect_with_retry(mock_pool):
    ...
    with patch("opengauss_mcp.sql.sql_driver.AsyncConnectionPool", return_value=mock_pool):
        with patch("opengauss_mcp.server.asyncio.sleep", AsyncMock()) as mock_sleep:
